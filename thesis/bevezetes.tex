...
Funkcionális nyelvek esetén azért érdemes gráfátíró rendszerekkel
foglalkozni, mert ezek ismeretében a programozó jobban átlátja a saját
kódját, tudni fogja, hogy pontosan mi történik a háttérben a programja
futása közben. Az adatgráfok és az átírás folyamatának ismerete segít
megbecsülni az adatcsúcsok számát a program futása közben, ami egyenes
arányban áll a szükséges memóriával; illetve a (késõbb részletesen
tárgyalt) redukciós lépések hasonlóképpen egyenes arányban állnak a
tényleges futási idõvel.

Ezt a modellt fogjuk használni a Haskell\footnote{Egy sokat ajánlott
  bevezetõ a nyelvbe, mely elsõsorban azoknak szól, akik még nem
  találkoztak funkcionális nyelvekkel: \cite{yaht}} esetében, ami egy lusta
funkcionális programozási nyelv\footnote{Egy bevezetõ a nyelv 98-as
  verziójába (mely az 1998-ban kiadott szabványt követi) olyanoknak,
  akik már találkoztak legalább egy funkcionális nyelvvel:
  \cite{tutorial98}}. A lustaság\footnote{A lustaság
  megnyilvánulásáról a Haskell programozási nyelv esetében
  \acite{laziness} ír} ebben az esetben azt
...
\begin{pelda}
  ...
  \begin{figure}[htb]
    \begin{center}
      \caption{Egy kételemû listához tartozó adatgráf \label{fig:lstdatag}}
      \includegraphics[height=0.3\textheight]{eps/grw_5.eps}
    \end{center}
  \end{figure}
\end{pelda}

Az adatgráfok nagy ereje abban rejlik, hogy tudunk velük úgynevezett
\emph{megosztott kifejezéseket} ábrázolni. Ez azt jelenti, hogy ha
például kétszer kell felhasználnunk egy kifejezésben egy
részeredményt, az nem kell, hogy kétszer szerepeljen az
adatgráfban. Másképpen megfogalmazva: a részeredményeket el tudjuk
tárolni adatgráfoknál.

\begin{pelda}
  Nézzünk tehát egy olyan kifejezést, aminél hasznos lehet a
  részeredmények eltárolása. Egy egyszerû példa erre a
  \begin{verbatim}
    (5+5)! * (5+5)!
  \end{verbatim}
  kifejezés, melyben kétszer lehetne kiszámolni az ötnek és az ötnek
  az összegét, majd ennek a faktoriálisát. Nézzük tehát ezt
  kétféleképpen adatgráfként ábrázolva: a részeredmények eltárolását
  \aref{fig:sharingyes} ábra mutatja, míg a naiv megoldást
  \aref{fig:sharingno} ábra.

  \begin{figure}[htb]
    \begin{center}
      \caption{Az (5+5)! * (5+5)! kifejezés adatgráfjai \label{fig:sharing}}
      \captionsetup{position=top}
      \subfloat[Megosztás nélkül]{
        \label{fig:sharingno}
        \includegraphics[height=0.3\textheight]{eps/sharing1.eps}
      }
      \vrule
      \subfloat[Megosztással]{
        \hspace{1cm}
...
\begin{pelda}
  ...
  \begin{figure}[htb]
    ...
    \begin{center}
      \subfloat[Elsõ sor\label{fig:eselso}]{
        \includegraphics{eps/grw_14.eps}
        \captionsetup{position=top}
      }
      \subfloat[Második sor\label{fig:esmasodik}]{
        \captionsetup{position=top}
